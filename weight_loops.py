...
def update_weight(file_path, feedback_map):
    ...
    if created:
        try:
            created_date = created if isinstance(created, datetime) else datetime.strptime(str(created), "%Y-%m-%d")
...
def main():
    ...
    for folder, type_ in [(PROGRAMS_PATH, "program"), (PROJECTS_PATH, "project"), (LOOP_PATH, "loop")]:
