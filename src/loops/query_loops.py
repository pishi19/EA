...
def log_results(query_text, results):
    ...
    with LOG_PATH.open("a") as log:
        ...
        for row in results:
            log.write(
                f"- **{row[0]}**: {row[1][:80]}... *(status: {row[2]}, priority: {row[3]})*\n"
            )
        log.write("</details>\n")
...
def main():
    ...
    parser.add_argument(
        "--status", type=str, help="Optional status filter: open/closed", default=None
    )
    ...
    for row in sqlite_data:
        print(
            f"- {row[0]} | {row[1][:80]}... | status: {row[2]} | priority: {row[3]} | weight: {row[4]}"
        )
