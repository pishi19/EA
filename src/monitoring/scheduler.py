...
class MonitoringScheduler:
                ...
                with open(state_file, 'r') as f:
                    ...
                    for task, data in state.items():
                        if task in self.schedules:
                            self.schedules[task]['last_run'] = datetime.fromisoformat(data['last_run'])
            except Exception as e:
                self.logger.error(f"Error loading schedule state: {str(e)}")
    ...
    def _save_schedule_state(self):
        ...
        try:
            ...
            self.logger.error(f"Error saving schedule state: {str(e)}")
    ...
    def run_log_rotation(self):
        ...
        if self._should_run('log_rotation'):
            try:
                ...
                self.logger.error(f"Error in log rotation: {str(e)}")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.logger.error(f"Unexpected error in scheduler: {str(e)}")
