...
class LogMonitor:
    def __init__(self, log_dir: str = "logs", metrics_file: str = "logs/metrics.json") -> None:
        self.log_dir = Path(log_dir)
        self.metrics_file = Path(metrics_file)
        self.log_dir.mkdir(exist_ok=True)

        # Component-specific thresholds
        self.thresholds: Dict[str, Dict[str, Union[float, int]]] = {
            "default": {
                "error_rate": 0.1,  # 10%
                "warning_rate": 0.2,  # 20%
                "max_size_mb": 10,  # 10MB
        ...
        self.historical_metrics: DefaultDict[str, List[Dict[str, Any]]] = defaultdict(list)
        self.metrics_retention_days: int = 30
        self.alert_history: DefaultDict[str, List[Dict[str, Any]]] = defaultdict(list)
        ...
        fh = handlers.RotatingFileHandler(
            "logs/monitor.log", maxBytes=1 * 1024 * 1024, backupCount=3  # 1MB
        )
        fh.setFormatter(
            logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")  # Simplified format
        )
        ...
        self.metrics: DefaultDict[str, Dict[str, Any]] = defaultdict(
            lambda: {
                "error_count": 0,
                "warning_count": 0,
                "info_count": 0,
        ...
        if self.metrics_file.exists():
            try:
                ...
                self.logger.error(f"Error loading existing metrics: {e!s}")
                self.logger.exception(e)  # Log the full traceback

    def _get_component_type(self, filename: str) -> str:
    ...
    def _get_thresholds(self, component: str) -> Dict[str, Union[float, int]]:
        """Get thresholds for a specific component"""
        return self.thresholds.get(component, self.thresholds["default"])

    def _update_growth_metrics(
        self, filename: str, current_size: int, metrics: Dict[str, Any]
    ) -> None:
        """Update growth-related metrics for a file"""
        if metrics["last_size"] > 0:
            growth = current_size - metrics["last_size"]
            growth_rate = growth / metrics["last_size"] if metrics["last_size"] > 0 else 0
            metrics["growth_rate"] = growth_rate
            metrics["growth_history"].append(growth_rate)

            # Keep only last 24 hours of growth history
            if len(metrics["growth_history"]) > 24:
                metrics["growth_history"] = metrics["growth_history"][-24:]

            # Update average and peak growth rates
            if metrics["growth_history"]:
                metrics["avg_growth_rate"] = sum(metrics["growth_history"]) / len(
                    metrics["growth_history"]
                )
                metrics["peak_growth_rate"] = max(metrics["growth_history"])
        ...
        for log_file in self.log_dir.glob("*.log*"):
            try:
                ...
                self.historical_metrics[log_file.name] = [
                    m
                    for m in self.historical_metrics[log_file.name]
                ...
                if metrics["error_count"] > 0:
                    ...
                    metrics["warning_count"] > 0 and metrics["warning_rate"] > 0.1
                ):  # Only log if warning rate > 10%
                    ...
                    self.logger.warning(
                        f"Rapid growth in {log_file.name} ({metrics['component']}): "
                        f"{metrics['growth_rate']:.2%} > {self.thresholds[metrics['component']]['growth_rate']:.2%}"
                    )

            except Exception as e:
                self.logger.error(f"Error collecting metrics for {log_file}: {e!s}")
                self.logger.exception(e)  # Log the full traceback
    ...
    def _save_metrics(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error saving metrics: {e!s}")
            self.logger.exception(e)  # Log the full traceback

    def _analyze_trends(self, filename: str, metrics: Dict[str, Any]) -> None:
    ...
    def _check_alerts(self) -> List[Dict[str, Any]]:
        ...
        for filename, metrics in self.metrics.items():
            ...
            if (
                current_time.timestamp() - self.last_alert_time[filename]
                < thresholds["alert_cooldown"]
            ):
                continue
            ...
            if metrics["error_rate"] > thresholds["error_rate"]:
                ...
                alerts.append(
                    {
                        "severity": severity,
                        "message": f"High error rate in {filename} ({component}): "
                        f"{metrics['error_rate']:.2%} > {thresholds['error_rate']:.2%}",
                        "component": component,
                        "metric": "error_rate",
            ...
            if metrics["warning_rate"] > thresholds["warning_rate"]:
                ...
                alerts.append(
                    {
                        "severity": severity,
                        "message": f"High warning rate in {filename} ({component}): "
                        f"{metrics['warning_rate']:.2%} > {thresholds['warning_rate']:.2%}",
                        "component": component,
                        "metric": "warning_rate",
            ...
            if metrics["growth_rate"] > thresholds["growth_rate"]:
                ...
                alerts.append(
                    {
                        "severity": severity,
                        "message": f"Rapid growth in {filename} ({component}): "
                        f"{metrics['growth_rate']:.2%} > {thresholds['growth_rate']:.2%}",
                        "component": component,
                        "metric": "growth_rate",
            ...
            if alerts:
                ...
                metrics["alert_history"].append(
                    {"timestamp": current_time.isoformat(), "alerts": alerts.copy()}
                )
    ...
    def _generate_visualizations(self) -> None:
        ...
        try:
            ...
            for component in set(df["component"]):
                ...
                axes[0, 1].bar(
                    component_files,
                    df.loc[component_files, "file_size"] / (1024 * 1024),
                )
            ...
            self.logger.error(f"Error generating visualizations: {e!s}")
            self.logger.exception(e)  # Log the full traceback

    def _generate_alert_report(self) -> None:
        ...
        try:
                ...
                try:
                    ...
                    for alert_entry in alert_history:
                        try:
                            ...
                            self.logger.warning(f"Error processing alert entry: {e!s}")
                            continue

                except Exception as e:
                    self.logger.warning(f"Error processing metrics for {filename}: {e!s}")
                    continue
            ...
            try:
                ...
                self.logger.warning(f"Error sorting alerts: {e!s}")
            ...
            for alert in all_alerts:
                try:
                    ...
                    self.logger.warning(f"Error counting alert statistics: {e!s}")
                    continue
            ...
            # Save report
            report_path = self.log_dir / "alert_report.txt"
            with open(report_path, "w") as f:
                f.write("\n".join(report))

        except Exception as e:
            error_msg = f"Error generating alert report: {e!s}"
            self.logger.error(error_msg)
            self.logger.exception(e)  # Log the full traceback
            report_path = self.log_dir / "alert_report.txt"
            with open(report_path, "w") as f:
                f.write(f"Error generating alert report: {error_msg}")

    def _standardize_session(self) -> None:
        """Standardize the current session's folder structure."""
        try:
            from scripts.standardize_session import (
                create_structure,
                move_logs,
                move_metrics,
            )
            ...
            self.logger.error(f"Error standardizing session: {e!s}")
    ...
    def _cleanup_old_logs(self) -> None:
        ...
        for log_file in self.log_dir.glob("*.log*"):
            try:
                ...
                self.logger.error(f"Error cleaning up {log_file}: {e!s}")

    def _rotate_large_logs(self) -> None:
        ...
        for log_file in self.log_dir.glob("*.log"):
            try:
                ...
                self.logger.error(f"Error rotating {log_file}: {e!s}")
    ...
    def _cleanup_old_backups(self, log_file: Path, max_backups: int):
        ...
        try:
            ...
            self.logger.error(f"Error cleaning up backups for {log_file}: {e!s}")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    self.logger.error(f"Error in monitoring cycle: {e!s}")
            ...
            self.logger.error(f"Unexpected error in log monitor: {e!s}")
