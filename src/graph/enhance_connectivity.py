...
def enhance_connectivity():
    with sqlite3.connect(GRAPH_DB) as conn:
        ...
        c.execute("SELECT id FROM nodes WHERE id NOT IN (SELECT from_id FROM edges) AND id NOT IN (SELECT to_id FROM edges)")
        isolated_nodes = c.fetchall()
        
        # Example: Connect isolated nodes to a central node (e.g., a project or loop)
        central_node = "02 Workstreams/Projects/project-2025-05-21-summary:-trial-users-are-exper.md"
        for node in isolated_nodes:
            c.execute("INSERT INTO edges (from_id, to_id, reason) VALUES (?, ?, ?)", (node[0], central_node, "enhanced_connectivity"))
