...
def main():
    ...
    with open(EXTRACT_LOG_PATH, "a") as log:
        if new_loops:
            memory.extend(new_loops)
            with open(MCP_MEMORY_JSON, "w") as f:
                json.dump(memory, f, indent=2)
            log.write(
                f"[{timestamp}] ✅ {len(new_loops)} new loop(s) added from {latest_note.name}\n"
            )
            logger.info(f"✅ {len(new_loops)} new loop(s) added from {latest_note.name}")
            os.system(f"python3 {MCP_MEMORY_JSON.parent / 'write_loops_to_files.py'}")
        else:
