...
def get_mcp_context(limit=5):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, summary, status, source, created FROM loops WHERE status = 'open' ORDER BY created DESC LIMIT ?",
            (limit,),
        )
...
def get_mcp_context_by_status(status="open", limit=5):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, summary, status, source, created FROM loops WHERE status = ? ORDER BY created DESC LIMIT ?",
            (status, limit),
        )
        loops = cursor.fetchall()
        return "\n\n".join(
...
def get_mcp_context_by_tag(tag="#loop", limit=5):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, summary, status, tags, source, created FROM loops ORDER BY created DESC"
        )
        loops = []
