...
with open(loop_path, "w", encoding="utf-8") as f:
    ...
    f.write(
        f"[✅ Mark loop as closed](shell:python3 ~/ea_assistant/mcp_server/interface/mcp_memory.py status {loop_id} closed)  \n"
    )
    f.write(
        f"[🔁 Reopen loop](shell:python3 ~/ea_assistant/mcp_server/interface/mcp_memory.py status {loop_id} open)\n\n"
    )

    f.write("### ✅ Verification\n\n")
    f.write(
        f"[🔏 Mark as verified](shell:python3 ~/ea_assistant/mcp_server/interface/mcp_memory.py verify {loop_id} true)  \n"
    )
    f.write(
        f"[🔓 Unverify](shell:python3 ~/ea_assistant/mcp_server/interface/mcp_memory.py verify {loop_id} false)\n"
    )
