...
- Name: Phase Doc View Page >> should load phase document page
- Location: /Users/air/Projects/ora-system/src/ui/react-app/e2e/phase-doc-view.spec.ts:8:7

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: getByText('Phase') resolved to 12 elements:
    1) <a href="/phase-doc" class="text-sm text-muted-foreground hover:text-primary">Phase Document</a> aka getByRole('link', { name: 'Phase Document' })
    2) <h1 class="text-4xl font-bold mb-2">Phase 8.2 – Yellow Slice: Contextual Execution</h1> aka getByRole('heading', { name: 'Phase 8.2 – Yellow Slice:' })
    3) <span class="text-lg text-gray-600">Phase 8.2</span> aka getByText('Phase 8.2', { exact: true })
    4) <label class="block text-sm font-medium text-gray-700 mb-1">Phase</label> aka getByText('Phase', { exact: true })
    5) <span>Phase 8.2: Phase 8.2 – Yellow Slice: Contextual E…</span> aka getByRole('combobox', { name: 'Select phase' })
    6) <h2 class="text-2xl font-bold mb-4">Phase Documentation</h2> aka getByRole('heading', { name: 'Phase Documentation' })
    7) <li>…</li> aka getByText('loop-2025-08-09-radix-testing-compat.md: Radix JSDOM issues deferred to Phase')
    8) <h2>🧾 Phase Update – 2025-06-07</h2> aka getByRole('heading', { name: '🧾 Phase Update – 2025-06-' })
    9) <li>…</li> aka getByText('Radix + JSDOM compatibility')
    10) <li>All testable components now covered; Ora is execu…</li> aka getByText('All testable components now')
    ...

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByText('Phase')

    at /Users/air/Projects/ora-system/src/ui/react-app/e2e/phase-doc-view.spec.ts:11:43
```
