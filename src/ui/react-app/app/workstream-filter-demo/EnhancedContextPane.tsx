...
export default function EnhancedContextPane({ 
    selectedNode, 
    selectedArtefact,
    className = "",
    onArtefactUpdate
    ...
    const loadChatHistory = async (artefact: Artefact) => {
        try {
            // In real implementation, this would fetch from persistent storage
    ...
    const addMemoryTraceEntry = (entry: Omit<MemoryTraceEntry, 'id' | 'timestamp'>) => {
    ...
    const simulateLLMStream = async (prompt: string, messageId: string): Promise<string> => {
        return new Promise((resolve, reject) => {
            if (!currentArtefact) {
                reject(new Error('No artefact selected'));
                return;
            }
            ...
            if (prompt.toLowerCase().includes('status')) {
                ...
                response = `I'll update "${currentArtefact.title}" to in_progress status.`;
            } else if (prompt.toLowerCase().includes('tag')) {
                response = `I can help you manage tags for "${currentArtefact.title}". Current tags: ${currentArtefact.tags.join(', ') || 'none'}. What tag would you like to add?`;
            } else if (prompt.toLowerCase().includes('summary')) {
                response = `Here's the current summary: "${currentArtefact.summary}". Would you like me to update it?`;
            } else {
            ...
            // Simulate streaming by revealing response gradually
            let index = 0;
