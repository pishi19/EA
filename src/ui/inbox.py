...
import json
from system.data_loader import get_inbox_entries
...
def render_inbox():
    ...
    inbox_items = get_inbox_entries()
    st.info(f"Loaded {len(inbox_items)} inbox entries")

    if not inbox_items:
        st.warning("No inbox entries found.")
    else:
        for item in inbox_items:
            with st.container():
                st.markdown(f"### {item['metadata'].get('title', 'Untitled')}")
                st.caption(f"📅 Created: {item['created'].strftime('%Y-%m-%d %H:%M')}")
                st.write(f"**Tags:** {', '.join(item['tags']) or '—'}")
                st.markdown(item['content'])
                st.divider()

def main():
    # st.set_page_config(page_title="EA Signal Inbox", layout="wide")  # Removed to avoid multiple calls
