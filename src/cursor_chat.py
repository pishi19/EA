...
class CursorChat:
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        avg_user_length = sum(self.message_lengths["user"]) / len(self.message_lengths["user"]) if self.message_lengths["user"] else 0
        avg_assistant_length = sum(self.message_lengths["assistant"]) / len(self.message_lengths["assistant"]) if self.message_lengths["assistant"] else 0
        ...
        return {
            ...
            "message_patterns": {
                ...
                "total_tokens_estimate": self.tokens_estimate["user"] + self.tokens_estimate["assistant"]
            },
            "files_modified": list(self.files_modified),
            "interaction_density": f"{total_messages / duration:.1f} messages/min" if duration > 0 else "0 messages/min"
        }
    ...
    def save_session(self, metadata: Dict[str, Any] = None):
        ...
        for message in self.messages:
            content += f"**{message['role'].title()}** ({message['timestamp']}):\n{message['content']}\n\n"
