...
class GmailIngest:
    ...
    def get_stats(self):
        ...
        return {
            ...
            "processed_today": sum(1 for thread_id in self.processed if thread_id.startswith(f"test-{datetime.now().strftime('%Y%m%d')}")),
            "pending_tasks": len(self.processed)  # This is a placeholder - we should count actual pending tasks
        }
