...
result = subprocess.run(
    ["pytest", "-v", "tests/test_system_integrity.py", "--tb=short", "--disable-warnings"],
    capture_output=True,
    text=True,
    env=env
)
...
def send_fix_prompt(file_path, error_output):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Test failure in {file_path}:\n\n{error_output}"}
        ]
    )
