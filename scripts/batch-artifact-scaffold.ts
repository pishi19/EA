...
// --- Roadmap Parsing ---
async function parseRoadmapForProject124(): Promise<RoadmapTask[]> {
  ...
  const taskMatches = project124Section.matchAll(
    /- Task (12\.4\.\d+): ([^\n]+)\s+- Description: ([^\n]+)/g
  );
...
// --- Artefact Generation ---
function generateArtefactTemplate(task: RoadmapTask): ArtefactTemplate {
  const uuid = uuidv4();
...
// --- File Operations ---
async function checkExistingArtefacts(tasks: RoadmapTask[]): Promise<RoadmapTask[]> {
...
async function createArtefactFiles(templates: ArtefactTemplate[]): Promise<void> {
...
async function updateExecutionLog(templates: ArtefactTemplate[]): Promise<void> {
  const roadmapContent = await fs.readFile(ROADMAP_PATH, 'utf-8');
  const today = new Date().toISOString().split('T')[0];
...
async function main(): Promise<void> {
  try {
    ...
    console.log('Do you want to proceed? (Type "yes" to continue, anything else to abort)');
    
    // For automated execution, we'll proceed by default
    // In interactive mode, you would await user input here
    const proceed = process.env.AUTO_APPROVE === 'true' || process.argv.includes('--auto-approve');
    
    if (!proceed) {
      console.log('\n⏸️  Execution paused. To proceed automatically, run with --auto-approve flag');
      console.log('   Example: tsx scripts/batch-artifact-scaffold.ts --auto-approve');
      return;
    }
