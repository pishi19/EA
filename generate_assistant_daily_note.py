...
with open(loop_path, "w", encoding="utf-8") as f:
    ...
    f.write(f"type: snapshot\n")
    f.write(f"topic: system_vision\n")
    f.write(f"tags: [#loop, #snapshot, #assistant]\n")
    f.write(f"source: obsidian:/0001 HQ/Daily Assistant/{today}-assistant.md\n")
    f.write(f"verified: false\n")
    f.write(f"---\n\n")

    f.write(f"### 🔁 Assistant Daily Snapshot\n")
    f.write(f"This loop captures the assistant system state on {today}.\n")
    f.write(f"Refer to [[System/Reference/EA-Vision]] for strategic alignment.\n\n")
